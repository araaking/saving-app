        ...
        <div class="card-body">
            <form action="{{ route('buku-tabungan.update', $bukuTabungan->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select id="siswa_id" name="siswa_id" class="form-select" required>
                        @foreach ($siswas as $siswa)
                            <option value="{{ $siswa->id }}" {{ $bukuTabungan->siswa_id == $siswa->id ? 'selected' : '' }}>
                                {{ $siswa->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select id="tahun_ajaran_id" name="tahun_ajaran_id" class="form-select" required>
                        @foreach ($tahunAjarans as $tahunAjaran)
                            <option value="{{ $tahunAjaran->id }}" {{ $bukuTabungan->tahun_ajaran_id == $tahunAjaran->id ? 'selected' : '' }}>
                                {{ $tahunAjaran->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" id="nomor_urut" name="nomor_urut" class="form-control" value="{{ $bukuTabungan->nomor_urut }}" required>
                </div>
